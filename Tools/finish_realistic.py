...
for fn in [f for f in os.listdir(SRC) if f.endswith('.png')]:
...
def local_position(m):
 src=node(m,u.MaterialExpressionPreSkinnedPosition);v=node(m,u.MaterialExpressionVertexInterpolator);lib.connect_material_expressions(src,'',v,'Input');return v
def const(m,v):
...
def custom(m,code,inputs,vec=True):
 ...
 return n
def save(m):lib.recompile_material(m);u.EditorAssetLibrary.save_loaded_asset(m)
